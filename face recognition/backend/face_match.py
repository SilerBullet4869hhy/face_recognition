...
def compare_faces_api(input_image):
    ...
    if best_match:
        # 返回匹配结果
        with open(best_match["image"], "rb") as img_file:
            match_base64 = base64.b64encode(img_file.read()).decode("utf-8")

        return {
            "match_name": best_match["name"],
            "match_image": f"data:image/jpeg;base64,{match_base64}",
            "similarity": round(similarity, 2)
        }
    else:
        return {"match_name": None, "match_image": None, "similarity": 0}
